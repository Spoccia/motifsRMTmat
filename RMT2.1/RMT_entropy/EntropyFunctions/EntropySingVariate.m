...
function [H_mean]= EntropySingVariate(X,thresh)%[H,H_mean ]= EntropySingVariate(X,thresh)

% Establish size of data
[n m] = size(X);

X(X<thresh)=0; %% the symbols <0  are  not considered
% Housekeeping
H = zeros(1,m);
...
%              Frequency = [Frequency,sum(X(:,Column) == Alphabet(symbol))];
%          else
%              zeroFreq = [Frequency,sum(X(:,Column) == Alphabet(symbol))];
%          end
    end
	
    % Calculate sample class probabilities
    divisor=sum(Frequency);%(sum([Frequency,zeroFreq]));
    %divisor=divisor+zeroFreq;
    P = Frequency / divisor;
...
% 
% function H = EntropySingVariate(X)
% 
% % Establish size of data
% [n m] = size(X);
% 
