%% This function is from paper RMT TOMM and create all aoctave and scales for the interval desired
%% The output is the list of festure from all octaves

function [tempFrames,descriptor_silv,gss,dogss,depd,idm, time, timee, timeDescr]=sift_gaussianSmooth_Proximity(I, LocM1 ,LocM2,LocM3, IDM1, IDM2,IDM3, Ot, Od, St, Sd, sigmaTime ,sigmaDepd, NBP, gthresh, r,sBoundary, eBoundary)
% [M,N,C] = size(I) ;
...
[gss, depd, idm] = gaussianss_Silv_Proximity(I, LocM1 ,LocM2,LocM3,IDM1, IDM2,IDM3, Ot,Od,St,Sd,otmin,odmin,stmin,sdmin,St+1,Sd+1, sigmaTime, sigmaDepd,gthresh,-1,-1);
...
%% decomment these 2 lines for working only in one octave 
 otime = Ot;
 odepd = Od;
%% decomment these 2 lines for working on all the octAVES 
...
%            %% SILV added a check if in scale 2 it is  even then  remove 1 to find the center
%             idxeven = rem(oframes(2,:),2)~=0;
%             y(idxeven)= y(idxeven)-1;
            tempDepd = oframes(1,:) ;
            tempTime = oframes(2,:) ;
