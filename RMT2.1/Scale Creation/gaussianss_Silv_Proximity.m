%% USE THIS FUNCTION IF YOU LIKE TO SUPERIMPOSE  DISTNCE MATRIX AND A CLUSTERING  FOR EACH OCTAVE
% LOCM* IS THE LOCATION PROXIMITY
% IDM* IS HOW VARIATE ARE  COMBINED IN CLUSTER IS USED TO MOOTH SND  DEFINE
% CLUSTER FOR NEXT OCTAVE
function [ SS,depd, IDMall] = gaussianss_Silv_Proximity(I,LocM1 ,LocM2,LocM3,IDM1, IDM2,IDM3,Ot,Od,St,Sd,ominT,ominD,sminT,sminD,smaxD,smaxT,sigmaT0,sigmaD0,DepThreshold,dsigmaT0,dsigmaD0)%
...
%% DECOMMENT TO WORK ONLY ON OCTAVE OT - OD
for i = 1:Ot
    for j = 1:Od
        if i ~= Ot && j ~= Od
            SS.octave{i,j}=[];
